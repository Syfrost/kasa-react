...
const StarsRating = ({ rating }) => {
    const stars = [];
    const numericRating = parseInt(rating, 10); // Convertit la chaîne en nombre

    // eslint-disable-next-line react/prop-types
    const Star = ({ filled }) => (
        <svg className="star" viewBox="5 5 26 26" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path
    ...
    for (let i = 1; i <= 5; i++) {
        stars.push(<Star key={i} filled={i <= numericRating}/>);
    }
